...
const ListaFinanceiro = ({ type }) => {
  ...
  const handleDelete = async (id, description, coletaId) => {
    try {
      ...
      // REGRA 3: Verificar se há pagamentos vinculados antes de excluir
      const { data: payments, error: paymentsError } = await supabase
        .from('pagamentos')
        .select('id')
        .eq('credito_debito_id', id);
      
      if (paymentsError) {
        throw new Error(`Erro ao verificar pagamentos: ${paymentsError.message}`);
      }
      
      // Se houver pagamentos, informar que precisa excluir os pagamentos primeiro
      if (payments && payments.length > 0) {
        toast({ 
          title: `Não é possível excluir ${title}`, 
          description: `Existem ${payments.length} pagamento(s) vinculado(s). Exclua os pagamentos antes de excluir o lançamento.`, 
          variant: 'destructive' 
        });
        return;
      }
