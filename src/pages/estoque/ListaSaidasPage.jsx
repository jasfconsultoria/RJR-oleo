...
import { Loader2, PlusCircle, Edit, Trash2, Search, ArrowUpSquare, ChevronUp, ChevronDown, Package } from 'lucide-react';
...
const ListaSaidasPage = () => {
  ...
  const fetchEntries = useCallback(async () => {
    ...
    if (entriesError) {
      toast({ title: 'Erro ao buscar saídas', description: `Falha na consulta: ${entriesError.message}`, variant: 'destructive' });
      setEntries([]);
      setTotalCount(0);
    } else if (countError) {
      toast({ title: 'Erro ao buscar contagem de saídas', description: `Falha na consulta de contagem: ${countError.message}`, variant: 'destructive' });
  ...
  const handleDelete = async (id, documentNumber) => {
    ...
    if (error) {
      ...
      await logAction('delete_stock_exit', { details: { exit_id: id, document_number: documentNumber } });
  ...
  return (
    <>
      <Helmet><title>Lista de Saídas - RJR Óleo</title></Helmet>
                ...
                {entries.length > 0 && (
                  <TableFooter>
                    <TableRow className="bg-black/20 font-bold text-white border-t-2 border-emerald-500 text-sm hidden md:table-row">
                      <TableCell colSpan={4} className="p-2">Total de Saídas (Período)</TableCell>
                      <TableCell className="text-right p-2">{formatNumber(summary.total_quantity_out)} kg</TableCell>
