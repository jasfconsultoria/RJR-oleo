...
-- Recreate the view with corrected logic for down payments (installment_number = 0)
...
SELECT
    ...
    cd.issue_date,
    cd.total_value,
    -- If it's a down payment (Entrada), the paid amount is the total value. Otherwise, calculate from payments.
    CASE
        WHEN cd.installment_number = 0 THEN cd.total_value
        ELSE COALESCE(ps.total_paid, 0::numeric)
    END AS paid_amount,
    -- If it's a down payment (Entrada), the balance is always 0. Otherwise, calculate.
    CASE
    ...
    cd.cost_center,
    cd.notes,
    -- If it's a down payment (Entrada), the status is always 'paid'. Otherwise, calculate.
    CASE
