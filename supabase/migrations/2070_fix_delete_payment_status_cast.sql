-- Correção da função delete_payment para usar o cast correto do tipo pagamento_status
-- Erro: column "status" is of type pagamento_status but expression is of type text
...
BEGIN
    ...
    -- Determine the new status with proper enum casting
    IF v_new_paid_amount <= 0 THEN
        v_new_status := 'pending'::public.pagamento_status;
    ELSIF v_new_paid_amount < v_credito_debito_entry.total_value THEN
        v_new_status := 'partially_paid'::public.pagamento_status;
    ELSE
        v_new_status := 'paid'::public.pagamento_status;
    END IF;
