...
DROP FUNCTION IF EXISTS create_financeiro_lancamento(uuid, text, text, text, uuid, text, text, text, text, text, text, uuid, integer, jsonb, jsonb[]);

-- Recreate the function with the correct type casting for payment_method
...
BEGIN
    ...
    IF p_down_payment IS NOT NULL THEN
        ...
        VALUES (
            ...
            p_description, (p_down_payment->>'date')::date, (p_down_payment->>'amount')::numeric, p_payment_method::public.payment_method, p_cost_center, p_notes, p_user_id,
        ...
        VALUES (
            down_payment_entry_id, (p_down_payment->>'amount')::numeric, (p_down_payment->>'date')::date, p_payment_method::public.payment_method, 'Pagamento de entrada.', p_user_id
        );
    END IF;
    ...
    IF array_length(p_installments, 1) > 0 THEN
        ...
        LOOP
            ...
            VALUES (
                p_lancamento_id, p_type, p_document_number, p_model, p_pessoa_id, p_cliente_fornecedor_name, p_cnpj_cpf,
                p_description, (inst->>'date')::date, (inst->>'amount')::numeric, p_payment_method::public.payment_method, p_cost_center, p_notes, p_user_id,
