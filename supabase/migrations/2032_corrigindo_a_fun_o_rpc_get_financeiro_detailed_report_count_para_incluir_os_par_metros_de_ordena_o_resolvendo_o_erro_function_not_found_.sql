CREATE OR REPLACE FUNCTION public.get_financeiro_detailed_report_count(
    p_start_date date,
    p_end_date date,
    p_type text,
    p_status text,
    ...
    p_cost_center text,
    p_sort_column text DEFAULT 'issue_date', -- Adicionado para corresponder à chamada do frontend
    p_sort_direction text DEFAULT 'desc'     -- Adicionado para corresponder à chamada do frontend
)
 RETURNS bigint
 LANGUAGE plpgsql
AS $function$
