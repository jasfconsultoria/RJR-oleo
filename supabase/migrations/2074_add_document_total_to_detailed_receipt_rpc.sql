-- Migração para adicionar a coluna valor_documento calculada à RPC get_financeiro_detailed_receipt
-- O objetivo é mostrar o valor total do lançamento (soma das parcelas) em todas as linhas da lista.
...
CREATE OR REPLACE FUNCTION public.get_financeiro_detailed_receipt(
 ...
 RETURNS TABLE(
    ...
    cliente_fornecedor_fantasy_name text, 
    cnpj_cpf text, 
    description text, 
    issue_date date, 
    total_value numeric, 
    valor_documento numeric, -- Nova coluna calculada
    valor_desconto numeric, -- Coluna de desconto que faltava no retorno
    paid_amount numeric, 
    amount_balance numeric, 
    payment_method text, 
    cost_center text, 
    notes text, 
    status public.pagamento_status, 
...
BEGIN
    RETURN QUERY
    WITH doc_totals AS (
        -- Calcular o total do documento (soma de todas as parcelas de um grupo)
        SELECT 
            cd_inner.lancamento_id, 
            SUM(cd_inner.total_value) as doc_totalValue,
            SUM(COALESCE(cd_inner.down_payment, 0)) as doc_totalDiscount -- Assumindo que down_payment ou outra coluna guarde desconto em versões futuras, por ora usaremos 0
        FROM public.credito_debito cd_inner
        GROUP BY cd_inner.lancamento_id
    )
    SELECT
        cd.id,
        cd.lancamento_id,
        ...
        cd.issue_date,
        cd.total_value,
        dt.doc_totalValue as valor_documento,
        COALESCE(cd.down_payment, 0) as valor_desconto, -- Usando down_payment como proxy para desconto se for o caso, ou apenas 0
        cd.paid_amount,
        cd.amount_balance,
        cd.payment_method,
        cd.cost_center,
    ...
    JOIN 
        doc_totals dt ON cd.lancamento_id = dt.lancamento_id
    LEFT JOIN
    ...
    ORDER BY
        ...
        cd.created_at DESC -- Fallback default order
    OFFSET p_offset
    LIMIT p_limit;
END;
$function$;
