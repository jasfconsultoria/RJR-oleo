-- Add the expected_total column to the pagamento table to store the total value of the debt at the time of payment.
ALTER TABLE public.pagamento
ADD COLUMN IF NOT EXISTS expected_total NUMERIC NOT NULL DEFAULT 0;

-- Update the RPC function to populate the new expected_total column upon payment registration.
...
BEGIN
    ...
    -- Insert the payment, including the total value at the time of payment
    INSERT INTO public.pagamento (credito_debito_id, paid_amount, payment_date, payment_method, notes, user_id, expected_total)
    ...
    -- Calculate the total paid for the entry
