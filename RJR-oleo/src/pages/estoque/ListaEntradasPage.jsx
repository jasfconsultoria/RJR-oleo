...
import { Loader2, PlusCircle, Edit, Trash2, Search, ArrowDownSquare, Eye } from 'lucide-react';
...
const ListaEntradasPage = () => {
  ...
  const fetchMovimentacoes = useCallback(async () => {
    ...
    let query = supabase
      ...
      `, { count: 'exact' })
      .eq('tipo', 'entrada') // Filter for entries only
  ...
  const handleDelete = async (id) => {
    ...
    if (error) {
      ...
      await logAction('delete_stock_entry', { movimentacao_id: id });
  ...
  return (
    <>
      <Helmet><title>Entradas de Estoque - RJR Óleo</title></Helmet>
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-white flex items-center gap-2">
                <ArrowDownSquare className="w-8 h-8 text-emerald-400" /> Entradas de Estoque
            </h1>
              ...
              <Table className="responsive-table">
                ...
                <TableBody>
                  {movimentacoes.length > 0 ? (
                      ...
                      <TableRow key={mov.id} className="border-b-0 md:border-b border-white/10 text-white/90 hover:bg-white/5 text-sm">
                        ...
                        <TableCell className="text-right actions-cell">
                           <div className="flex justify-end items-center gap-2">
                            ...
                            <Button variant="ghost" size="icon" className="text-yellow-400 hover:text-yellow-300 rounded-xl" onClick={() => navigate(`/app/estoque/entradas/editar/${mov.id}`)}><Edit className="h-4 w-4" /></Button>
                            <AlertDialog>
                              ...
                              <AlertDialogContent className="bg-emerald-900 border-emerald-700 text-white rounded-xl">
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription className="text-emerald-300">Essa ação não pode ser desfeita. Isso deletará permanentemente a entrada de estoque.</AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                    ...
                    <TableRow><TableCell colSpan="5" className="text-center text-gray-400 py-10">Nenhuma entrada encontrada.</TableCell></TableRow>
                  )}
                </TableBody>
              </Table>
            )}
